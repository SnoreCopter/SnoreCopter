## 
## Makefile for AeroQuad Maple
## 

MAPLE_LIB_HOME = libmaple
AEROQUAD_LIB = Libraries
USER_INCLUDES = AeroQuad AeroQuad32 MapleCompatibility $(AEROQUAD_LIB)/AQ_Accelerometer $(AEROQUAD_LIB)/AQ_BarometricSensor $(AEROQUAD_LIB)/AQ_BatteryMonitor \
 $(AEROQUAD_LIB)/AQ_CameraStabilizer $(AEROQUAD_LIB)/AQ_Compass $(AEROQUAD_LIB)/AQ_Defines \
 $(AEROQUAD_LIB)/AQ_FlightControlProcessor $(AEROQUAD_LIB)/ $(AEROQUAD_LIB)/AQ_Gps $(AEROQUAD_LIB)/AQ_Gyroscope \
 $(AEROQUAD_LIB)/AQ_Kinematics $(AEROQUAD_LIB)/AQ_Motors $(AEROQUAD_LIB)/AQ_SPI \
 $(AEROQUAD_LIB)/AQ_OSD  $(AEROQUAD_LIB)/AQ_Platform_APM $(AEROQUAD_LIB)/AQ_Platform_CHR6DM \
 $(AEROQUAD_LIB)/AQ_Platform_MPU6000 $(AEROQUAD_LIB)/AQ_Platform_Wii $(AEROQUAD_LIB)/AQ_RangeFinder \
 $(AEROQUAD_LIB)/AQ_Receiver $(AEROQUAD_LIB)/AQ_SPI $(AEROQUAD_LIB)/AQ_RSSI $(AEROQUAD_LIB)/AQ_SoftModem \
 $(AEROQUAD_LIB)/AQ_RSCode $(AEROQUAD_LIB)/AQ_Math $(AEROQUAD_LIB)/AQ_I2C $(AEROQUAD_LIB)/AQ_Math .
 #SoftwareSerial  .
USER_MODULES = $(AEROQUAD_LIB)/AQ_Math MapleCompatibility  
...
screen:
	@echo $(shell $(SUPPORT_PATH)/scripts/reset.py | awk -F '[ ]' '{print $2}')
#	$(SUPPORT_PATH)/scripts/reset.py | awk -F '[ ]' '{print $2}'  
